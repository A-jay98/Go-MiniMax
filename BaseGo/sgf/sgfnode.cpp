...
{
  SGFNode *newnode;
  newnode = static_cast<SGFNode*>(xalloc(sizeof(SGFNode)));
  newnode->next = NULL;
  newnode->props = NULL;
  newnode->parent = NULL;
  newnode->child = NULL;
...
sgfOverwriteProperty(SGFNode *node, const char *name, const char *text)
{
  ...
  for (prop = node->props; prop; prop = prop->next)
    if (prop->name == nam) {
      prop->value = static_cast<char*>(xrealloc(prop->value, strlen(text)+1));
...
sgfOverwritePropertyInt(SGFNode *node, const char *name, int val)
{
  ...
  for (prop = node->props; prop; prop = prop->next)
    if (prop->name == nam) {
      prop->value = static_cast<char*>(xrealloc(prop->value, 12));
...
sgfOverwritePropertyFloat(SGFNode *node, const char *name, float val)
{
  ...
  for (prop = node->props; prop; prop = prop->next)
    if (prop->name == nam) {
      prop->value = static_cast<char*>(xrealloc(prop->value, 15));
		     ...
		     SGFNode *node, SGFProperty *last)
{
  ...
  prop->name = sgf_name;
  prop->value = static_cast<char*>(xalloc(strlen(value) + 1));
...
{
  char *str = static_cast<char*>(xalloc(strlen(text) + 3));
